...
/**
 * Known-answer tests for DER -> IEEE P1363 on the Android side.
 *
 ...
 * encoding step, which is where "valid ECDSA signature that the BFF rejects"
 ...
 * `"flutter_sign_keypair known-answer vector"`, picked out of 1200 samples to
 ...
 * `openssl dgst -sha256 -verify` against the real public key.
 *
 * These are the **same vectors** used by the Swift suite
 * (`darwin_tests/.../EcdsaSignatureCodecTests.swift`) and the Dart suite
 * (`test/asn1_test.dart`), so the three implementations cross-validate.
 */
class DerToP1363Test {
    ...
    fun `real signature with short r is left padded`() {
        ...
        val actual = FlutterSignKeypairPlugin.derToP1363(der)
    ...
    fun `tiny integers are padded on the left`() {
        val actual = FlutterSignKeypairPlugin.derToP1363(hex("3006020101020102"))
    ...
    @Test
    fun `zero component`() {
        val actual = FlutterSignKeypairPlugin.derToP1363(hex("3006020100020101"))
    ...
    fun `accepts the long-form SEQUENCE length`() {
        ...
        assertContentEquals(r + s, FlutterSignKeypairPlugin.derToP1363(der))
    }
    ...
    fun `output is always exactly 64 bytes`() {
        ...
        for (input in inputs) {
            assertEquals(64, FlutterSignKeypairPlugin.derToP1363(hex(input)).size, "input $input")
        }
    }
    ...
    fun `malformed inputs are rejected`() {
        ...
        for ((input, description) in cases) {
            assertFailsWith<IllegalArgumentException>("expected a throw for $description") {
                FlutterSignKeypairPlugin.derToP1363(hex(input))
            }
        }
    }
     ...
     */
    @Test
    fun `all zero-length integers are rejected`() {
        assertFailsWith<IllegalArgumentException> {
            FlutterSignKeypairPlugin.derToP1363(hex("300402000200"))
        }
    }

    /**
     * Trailing bytes must be rejected: silently ignoring them makes the encoding
     * malleable (two distinct byte strings, one signature). `openssl asn1parse`
     * rejects this same input with "Error in encoding".
     */
    @Test
    fun `trailing bytes after s are rejected`() {
        assertFailsWith<IllegalArgumentException> {
            FlutterSignKeypairPlugin.derToP1363(hex("3009020101020102aabbcc"))
        }
    }
}
