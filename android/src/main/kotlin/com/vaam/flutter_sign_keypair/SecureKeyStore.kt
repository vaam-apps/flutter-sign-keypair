...
/**
 * All AndroidKeyStore interaction, with no Flutter dependency.
 *
 * Split out of [FlutterSignKeypairPlugin] so the instrumented tests can drive
 * **the production code path** rather than a copy of its `KeyGenParameterSpec`.
 * That distinction matters: a test that rebuilds the spec itself would keep
 * passing if the real spec drifted to something exportable, which is precisely
 * the regression the tests exist to catch.
 *
 ...
 *   `KeyguardManager`, and taking a `Context` here would put this class back
 *   out of reach of the instrumented tests that drive the production spec.
 */
internal class SecureKeyStore(
    ...
    fun generateKey(
        ...
        // re-enrolment. For a device-bound banking key that is a far worse
        // outcome than the failure the caller asked for. The probe below uses a
        // throwaway alias, so it costs nothing but a keygen.
        ...
        // the caller distinguish "set a screen lock" (the customer can fix this
        // in Settings) from "no secure element" (they cannot).
        if (protection.requiresUserPresence && !userAuthenticationConfigured()) {
            throw SignerError(
        ...
        if (store.containsAlias(keyId)) {
            if (!overwrite) {
                throw SignerError(SignerErrorCode.KEY_ALREADY_EXISTS, "A key already exists under \"$keyId\"")
            }
        ...
        // loudly rather than hand back a software key to a caller that asked
        // for hardware — silently degrading would be worse than an error,
        // because the caller would log "hardware-backed" for a key that is not.
        //
        ...
        // an alias, so generate-then-swap is not available; this path is
        // narrow (device advertises hardware, then refuses) and the pre-check
        ...
        return describeKey(keyId)
            ?: throw SignerError(SignerErrorCode.KEYSTORE_FAILURE, "Key vanished immediately after creation")
    }
    ...
    fun describeKey(keyId: String): Description? {
        ...
        return Description(
            keyId = keyId,
            x = base64Url(coordinateBytes(point.affineX)),
            y = base64Url(coordinateBytes(point.affineY)),
     ...
     * through the prompt, and come back to be finished. Splitting the two halves
     * is the only shape that supports both.
     */
    fun beginSign(keyId: String): Signature {
        ...
        return Signature.getInstance(SIGNATURE_ALGORITHM).apply {
            try {
                ...
                // ADR 0024 §5 firing as designed: a biometric was re-enrolled and
                // the keymaster destroyed this key. Reported distinctly because
                // the recovery is narrow — generate a fresh user-present key and
                // register its thumbprint. The ambient key is untouched, so the
                // device can still authenticate itself while doing so, and
                // treating this as a generic keystore failure would send the
                // customer through the full ADR 0017 SMS ceremony for nothing.
                throw SignerError(
    ...
    fun finishSign(signature: Signature, payload: ByteArray): ByteArray {
        ...
        return FlutterSignKeypairPlugin.derToP1363(signature.sign())
    }
     ...
     * this is the key the Dio interceptor signs every request with, and requests
     * are generated by background timers (`Timer.periodic(5s)` in mobile-money
     * and top-up, a 3s tick while an animated QR renders) in contexts where no
     * prompt can be displayed at all.
     *
     * [KeyProtection.USER_PRESENT] is ADR 0024's tier-3 key and binds three
     * things:
     *
     ...
     *    stronger than the app-lock in `lib/core/biometric/`, which only decides
     *    whether to draw a screen and which anything running in-process walks
     *    straight past.
     *  - a **zero** timeout with `AUTH_BIOMETRIC_STRONG or AUTH_DEVICE_CREDENTIAL`
     *    — per-operation authorisation, and the device credential accepted
     *    alongside biometrics because a meaningful share of the target hardware
     *    has no working sensor (ADR 0024 §6). Requiring biometrics specifically
     *    would exclude those customers from tier 3 entirely.
     *  - `setInvalidatedByBiometricEnrollment(true)` — ADR 0024 §5. A newly
     *    enrolled fingerprint must not inherit this key's authority.
     *
     * Be precise about what that last flag buys here, because it is less than
     * §5 alone suggests: Android does not invalidate a key that also accepts the
     * device credential, so in this combination the flag is a statement of
     * intent that only takes effect on the biometric branch. It is not load
     * bearing on its own — what actually protects the key is that enrolling a
     * biometric requires the existing credential first, so an attacker who can
     * add their fingerprint already holds what the surviving branch asks for.
     *
            ...
            .apply {
                ...
                if (protection.requiresUserPresence) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                        ...
                        // key is **biometric-only**, so ADR 0024 §6's "device
                        // credential is an acceptable authenticator" does not
                        // hold on those releases. A passcode-only device on API
                        // 24-29 cannot create one, and `userAuthenticationConfigured`
                        // above will not catch that — the keygen below throws
                        // and surfaces as a keystore failure. Accepted rather
                        // than papered over: the platform offers no per-operation
                        // credential-backed key before R.
                        @Suppress("DEPRECATION")
    ...
    /** Whether the device has anything the keystore would accept as a human. */
    private fun userAuthenticationConfigured(): Boolean = userAuthenticationAvailable()
    ...
    private fun coordinateBytes(value: BigInteger): ByteArray =
        FlutterSignKeypairPlugin.coordinateBytes(value)

    internal class SignerError(val code: SignerErrorCode, override val message: String) : Exception(message)
    ...
    companion object {
         ...
         *
         * The alternative, a short window, is ADR 0024's one deliberately
         * unresolved question: kinder to a multi-step flow like a transfer with
         * a confirmation screen, weaker against an attacker who wins the race
         * inside the window. Per-operation is the stronger default and the one
         * to move away from only with measurement against real flows.
         */
        ...
        private const val PROBE_ALIAS_PREFIX = "__flutter_sign_keypair_backing_probe_"
    }
}
