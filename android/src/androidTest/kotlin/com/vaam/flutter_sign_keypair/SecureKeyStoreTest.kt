 ...
 *    in the app process. This is the property that replaces the old
 *    `keyPair.extract()` path, and it is genuinely proven here.
...
class SecureKeyStoreTest {
    ...
    private val keyId = "flutter_sign_keypair_instrumented_test"
     ...
     *
     * This is the security upgrade in one assertion: the app's previous Dart
     * path called `keyPair.extract()` and got 32 bytes of private scalar into
     * heap memory. Here there is nothing to extract — `encoded` is null, and no
     * method on [SecureKeyStore] returns key material.
     */
    @Test
     ...
     * BFF, so a JWK-encoding bug would surface here too.
     */
    @Test
